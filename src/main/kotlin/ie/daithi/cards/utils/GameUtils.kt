...
class GameUtils(
    ...
    fun findWinners(game: Game): List<Player> {
        ...
        if (!isGameOver(game.players)) throw GameNotOverException("Game isn't over")
    ...
    fun canRenage(leadOut: Card, myTrumps: List<Card>): Boolean {
        myTrumps.forEach { if (!it.renegable || it.value <= leadOut.value) return false }
        return true
    }
    ...
    fun orderPlayersAtStartOfGame(dealerId: String, players: List<Player>): List<Player> {
        ...
        val playersSansDummy = if (dummy != null) players.minus(dummy) else players
        ...
        for (x in 0 until playersSansDummy.size - 1) {
            currentIndex = if (currentIndex < 1) playersSansDummy.size - 1 else currentIndex - 1
    ...
    fun orderPlayers(currentPlayer: Player, players: List<Player>): List<Player> {
        ...
        for (x in 0 until playersSansDummy.size - 1) {
            currentIndex = if (currentIndex >= playersSansDummy.size - 1) 0 else currentIndex + 1
            playerStack.push(playersSansDummy[currentIndex])
        }
