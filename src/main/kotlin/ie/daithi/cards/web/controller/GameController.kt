...
class GameController(
    ...
    @ApiResponses(
        ...
        ApiResponse(
            responseCode = "502",
            description = "An error occurred when attempting to create new game"
        )
    )
    @ResponseBody
    ...
    @Operation(
        summary = "Make a call",
        description = "After dealing each player can make a call between 0-30"
    )
    @ApiResponses(ApiResponse(responseCode = "200", description = "Request successful"))
    fun call(@RequestParam gameId: String, @RequestParam call: Int) {
        ...
        if (!game.players.map { player -> player.id }.contains(user.id!!))
            throw ForbiddenException("Can only call if you are part of the game.")
    ...
    fun buyCards(@RequestParam gameId: String, @RequestBody cards: List<Card>) {
        ...
        // 3. Check the player is in this game
        if (!game.players.map { player -> player.id }.contains(user.id!!))
            throw ForbiddenException("Can only buy cards if you are part of the game.")
    ...
    fun chooseFromDummy(
        ...
        // 3. Check the player is in this game
        if (!game.players.map { player -> player.id }.contains(user.id!!))
            throw ForbiddenException("Can only choose from the dummy if you are part of the game.")

        // 4. Choose from dummy
        gameService.chooseFromDummy(
            gameId = game.id,
            playerId = user.id!!,
            selectedCards = cards,
            suit = suit
    ...
    fun playCard(@RequestParam gameId: String, @RequestParam card: Card) {
        ...
        // 3. Check the player is in this game
        if (!game.players.map { player -> player.id }.contains(user.id!!))
            throw ForbiddenException("Can only play card if you are part of the game.")
    ...
    fun replay(@RequestParam gameId: String) {
        ...
        // 3. Check the player is in this game
        if (!game.players.map { player -> player.id }.contains(user.id!!))
            throw ForbiddenException("Can only replay game if you are part of the game.")
